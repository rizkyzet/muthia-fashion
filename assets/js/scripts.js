...
$(document).on("change", "#checkout_ongkir", function () {
	...
	$(".total_bayar").text(rubah(total_bayar));
	...
	if ($("#checkout_form").valid()) {
		...
		$.ajax({
			...
			data: {
				...
				layanan: layanan,
				shipping_kota: shipping_kota,
				shipping_alamat: shipping_alamat,
				shipping_no_tlp: shipping_no_tlp,
				shipping_nama: shipping_nama,
				shipping_kode_pos: shipping_kode_pos,
			},
			method: "POST",
			cache: false,
...
$(document).ready(function () {
	$(".update_stok").on("click", function () {
		...
		$("#" + id + "").on("change", function () {
			...
			$.ajax({
				url:
					"http://localhost/muthia-fashion/admin/detail_barang/ajax_detail_stok",
				data: {
					stok: stok,
					id: id,
					kd_brg: kd_brg,
				...
				success: function () {
					document.location.href =
						"http://localhost/muthia-fashion/admin/detail_barang/tampil/" +
						kd_brg +
						"";
				},
			});
...
$(document).ready(function () {
	$("#tanggal_awal").on("change", function () {
		...
		$.ajax({
			url:
				"http://localhost/muthia-fashion/pemilik/laporan/ajax_laporan_penjualan/",
			data: {
